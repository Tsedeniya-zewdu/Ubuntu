...
export const ReportsCompFundraiser = () => {
  ...
  useEffect(() => {
    const getCompleted = async () => {
      ...
      try {
        ...
        res.data.forEach((data) => {
          if (data.fundraiser == currentUser._id) {
            tmp.push(data)
          }
        })
        setCompleted(tmp.length)
      } catch (err) {
        console.log(err)
      }
    }
    ...
    const getDonations = async () => {
      ...
      try {
        ...
        res.data.forEach((data) => {
          if (data.fundraiserId == currentUser._id) {
      ...
      datasets: [
        {
          ...
          backgroundColor: ['#8cf0f1', '#60af95', '#43ba2f', '#2a71d0'],
        },
      ],
    }
